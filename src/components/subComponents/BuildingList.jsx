...
const BuildingList = ({campus}) => {
    ...
    const fetchList = async()=>{
        const response = await axios.post('http://localhost:5000/ct-api/buildingList',{
            campus
        })
    ...
    const addBuilding = async (e) =>{
         ...
         try {
            const response = await axios.post('http://localhost:5000/ct-api/addBuilding',{
                campus,
                description
            })
        ...
        <div className="campus-staff-main">
            <div className="list-top-controls">
                <div className="list-counter-box">
                    ...
                    <div className="list-counter-label">
                         Total Buildings
                    </div>
                </div>
                <div className="list-filter-controls">
                    ...
                    <form onSubmit={addBuilding}>
                        <div className="form-inputs">
                            ...
                            <input minLength={'5'} required value={description} onChange={(e)=>setDescription(e.target.value)} type="text" className="form-control" />
                            </div>
                        </div>
                        <div className="px-2 mt-4 text-end">
                            <button type='submit' className="btn btn-primary"><i className="me-2 fas fa-save"></i>Save</button>
                        </div>
                    </form>
                </div>
            </div>
             <div className="table-list staff-table py-5">
                <div className="table-header px-2 mb-5 d-flex justify-content-between align-items-center">
                    ...
                    <div className="input-group">
                        <input placeholder='Description'   type="search" className="form-control search-control rounded"  aria-label="Search" aria-describedby="search-addon" />
                           ...
                           <td>{list.description}</td>
                            <td>
                                 <div  className="table-options justify-content-center">
                                    <span className='option-edit me-3'><i className="fas fa-edit"></i></span>
                                    <span className='option-delete'><i className="fas fa-trash-alt"></i></span>
                                </div>
                            </td>
                        </tr>
                        )):
            ...
            <div className="show-count p-4">Showing <span className='text-primary fw-bold'>{filterList.length}</span> out of <span className='text-primary fw-bold'>{filterList.length}</span> Records</div>
            </div>
        </div>
    </div>
  )
