           ...
           <div className="admin-main">
              <div className="admin-main-controls">
                 <div className="group d-flex align-items-center justify-content-center">
                    ...
                    <select onChange={()=>setShowAll(!showAll)} className='form-control m-0'>
                 ...
                 <div className="group d-flex align-items-center justify-content-center">
                    ...
                    <select className='form-control m-0' onChange={(e)=>setCampusState(e.target.value)}>
                        {
                           campusList.map(list=>(
                              <option key={list.id} id={list.id} value={list.text}>{list.text}</option>
                           ))
                        }
                    </select>
                 </div>
                 <div className="group d-flex align-items-center justify-content-center">
                    <label className='select-label'>Search<i className="fas fa-search ms-2"></i></label>
                     <input type="text" className="form-control top-field-control m-0" />
                 </div>
              </div>
              ...
              <table className="campus-tables table table-striped">
                <thead>
                    ...
                    <th className='fw-bold' scope="col">Full Name</th>
                    <th className='fw-bold' scope="col">Campus</th>
                    <th className='fw-bold' scope="col">Department</th>
                    <th className='fw-bold' scope="col">ID Number</th>
                ...
                <tbody>
                {
                    ...
                    staff.map(list=>(
                        ...
                        <td><span className='me-2'>{list.firstName}</span>{list.lastName}</td>
                        <td>{list.campus}</td>
                        ...
                        <td>
                           {
                              list.isAdmin === 1 ? <div className="form-check form-switch">
                              <input onChange={()=>changeStatus()} className="form-check-input" type="checkbox" role="switch"  defaultChecked={true} />
                            </div> :  <div className="form-check form-switch">
                              <input onChange={()=>changeStatus()} className="form-check-input" type="checkbox" role="switch"  defaultChecked={false} />
                            </div>
                           }
                        ...
                        <td>
                            <div className="table-options">
                                <span onClick={()=>deleteStaff(list.id)} className='option-delete'><i className="fas fa-trash-alt"></i></span>
                            </div>
                        </td>
                        </tr>
                    ))
