...
const StaffAttendanceRecord = ()=>{
    ...
    useEffect(()=>{
        const sendRequest = async()=>{
            const sections = await fetchData('http://localhost:5000/records/sectionList');
            const attendance = await fetchData('http://localhost:5000/records/attendanceList');
    ...
    const filterSections = (id)=>{
        ...
        setSectionFilter(sectionList.filter(list=>(list.subjectId.toString() === id)));
    ...
    const filterAttendance = (sectionId)=>{
        ...
        setAttendanceFilter(attendanceList.filter(list=>(list.sectionId.toString() === sectionId && list.subjectId.toString() === subjectId)));
    ...
    return(
        ...
        <div className='staff-attendance-record-container'>
           <div className="attendance-record-title"><i className="me-3 far fa-file-archive"></i>Attendance Records</div>
           {
             ...
             <div className="attendance-options">
                <div className="attendance-option-left">
                    <div className="form-group">
                        ...
                        <select onChange={(e)=>filterSections(e.target.value)} className="form-control-select" defaultValue={''}>
                    ...
                    <div className="form-group mt-3">
                        ...
                        <select onClick={(e)=>filterAttendance(e.target.value)} className="form-control-select" defaultValue={''}>
                            <option value="" disabled>Select</option>
                            {
                                sectionFilter.map(list=>(
                                    <option key={list.id} value={list.value}>{list.text}</option>
                                ))
                        ...
                        <tr key={list.id} >
                            ...
                            <td>
                                <div className="table-options">
                                    <span onClick={()=>setPreview(!preview)} className='mx-2 controls option-view text-warning'><i className="fas fa-eye"></i></span>
                                    <span onClick={()=>printPage()}  className='mx-2 controls option-download text-primary'><i className="fas fa-download"></i></span>
                                    <span onClick={()=>removeList(list.id)}  className='mx-2 controls option-delete'><i className="fas fa-trash-alt"></i></span>
                                </div>
                            </td>
                        </tr>
                        ))
